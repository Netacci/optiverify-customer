...
export default function ManagedServiceDetailsPage({
  ...
  const {
    ...
    queryKey: ["managedService", id],
    queryFn: () => getManagedServiceDetails(id),
  ...
  const syncPaymentMutation = useMutation({
    mutationFn: () => syncManagedServicePayment(id),
    onSuccess: (data) => {
      if (data.success) {
  ...
  const savingsFeePaymentMutation = useMutation({
    mutationFn: () => createSavingsFeePaymentSession(id),
    onSuccess: (data) => {
  ...
  const updateMutation = useMutation({
    mutationFn: (data: typeof editFormData) => updateManagedService(id, data),
    onSuccess: () => {
      toast.success("Request updated successfully");
      setIsEditing(false);
      queryClient.invalidateQueries({ queryKey: ["managedService", id] });
    },
    onError: (error: unknown) => {
              ...
              <div className="space-y-6">
                ...
                request.savingsFeeStatus !== "paid" ? (
                  ...
                  <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
                    ...
                    <div className="mt-6 pt-6 border-t border-gray-200">
                      <Link
                        href={`/dashboard/feedback?type=matching_service&matchingServiceId=${id}`}
