...
export default function RequestDetailsPage() {
  ...
  // intent explicit.
  //
  // Matching redesign: when matches are already generated by AI Call 1 at
  // request creation, the backend's getRequestDetails returns the populated
  // suppliers and lazy-fires Call 2 (prose) itself — no need to call the
  // legacy /generate-match endpoint. Skip the auto-fire if suppliers are
  // already populated.
  const generateFiredRef = useRef(false);
  useEffect(() => {
    const alreadyHasMatches = (suppliers?.length ?? 0) > 0;
    if (
      isUnlocked &&
      !alreadyHasMatches &&
  ...
  }, [isUnlocked, generateMatchMutation, suppliers]);
  ...
  // No-matches state (matching redesign §5.4): AI scored every category-
  // matched supplier but none cleared the 80 threshold. Show a CTA into
  // the managed-services flow instead of an empty report.
  if (isNoMatches) {
    return (
      <>
        <Head>
          <title>No Strong Matches - Optiverifi</title>
        </Head>
        <DashboardLayout>
          <div className="max-w-3xl mx-auto py-12 px-4">
            ...
            <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-10 text-center">
              ...
              <h1 className="text-3xl font-bold text-gray-900 mb-3">
                No strong matches in our network
              </h1>
              <p className="text-gray-600 max-w-xl mx-auto mb-2">
                We searched our network of suppliers in{" "}
                ...
                but none were a strong fit for{" "}
                <span className="font-semibold text-gray-800">
                  {request?.name}
                </span>
                .
              ...
              {requestSummary && (
                <p className="text-sm text-gray-500 italic max-w-xl mx-auto mb-8">
                  &ldquo;{requestSummary}&rdquo;
                </p>
              )}
              <p className="text-gray-600 max-w-xl mx-auto mb-8">
                Our managed sourcing team can hand-curate suppliers for
                hard-to-find products that aren&apos;t in our matching
                network yet.
              </p>
              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                <Link
                  href={`/managed-services?fromRequest=${requestId}`}
                  className="inline-flex items-center justify-center gap-2 py-3 px-6 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 transition-colors shadow-lg shadow-blue-200"
                >
                  Request managed sourcing
                  <span aria-hidden="true">&rarr;</span>
                </Link>
                <Link
                  href="/submit-request"
                  className="inline-flex items-center justify-center gap-2 py-3 px-6 bg-white border border-gray-300 text-gray-700 rounded-lg font-semibold hover:bg-gray-50 transition-colors"
                >
                    ...
                    {suppliers.map((supplier: Supplier, index: number) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-4">
                          ...
                          {/* Badges row — verified + AI matching fit signals */}
                          <div className="flex flex-wrap gap-1.5 mt-1">
                            {supplier.verified && (
                              <span className="inline-flex items-center text-[10px] px-1.5 py-0.5 rounded bg-green-50 text-green-700 font-medium">
                                Verified
                              </span>
                            )}
                            {supplier.meetsMoq === true && (
                              <span
                                className="inline-flex items-center text-[10px] px-1.5 py-0.5 rounded bg-blue-50 text-blue-700 font-medium"
                                title="Meets your minimum order quantity"
                              >
                                ✓ MOQ
                              </span>
                            )}
                            {supplier.meetsCompliance === true && (
                              <span
                                className="inline-flex items-center text-[10px] px-1.5 py-0.5 rounded bg-blue-50 text-blue-700 font-medium"
                                title="Meets your compliance requirements"
                              >
                                ✓ Compliance
                              </span>
                            )}
                          </div>
                          {/* AI scoring rationale (matching redesign) */}
                          {supplier.reason && (
                            <p className="text-xs text-gray-500 mt-1 max-w-md line-clamp-2">
                              {supplier.reason}
                            </p>
                          )}
                        </td>
